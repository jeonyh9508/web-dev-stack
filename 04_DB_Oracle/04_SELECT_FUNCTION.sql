...
SELECT EMP_NAME,JOB_CODE,SALARY,
    DECODE(JOB_CODE,'J7',SALARY*1.1,'J6',SALARY*1.15,'J5',SALARY*1.2,SALARY*1.05) 답,
    DECODE(JOB_CODE,'J7',SALARY*1.1,'J6',SALARY*1.15,'J5',SALARY*1.2) "J5,J6,J7",
    NVL(DECODE(JOB_CODE,'J7',SALARY*1.1,'J6',SALARY*1.15,'J5',SALARY*1.2),SALARY*1.05) 인상급여
FROM EMPLOYEE
ORDER BY JOB_CODE ASC, 인상급여 DESC;
...
SELECT EMP_NAME, SALARY,
    CASE WHEN 5000000 < SALARY THEN '1등급'
         WHEN 3500000 < SALARY THEN '2등급' -- 위 값은 적용된 상황 : 500만원 이하
         WHEN SALARY BETWEEN 2000000 AND 3500000 THEN '3등급'  -- BETWEEN 사용 가능
         ELSE '4등급'
         END 등급
FROM EMPLOYEE
...
*/
-- SUM : 해당 컬럼 값들의 총 합계
-- USER_INFO에서 나이(AGE)를 모두 더한 값
SELECT SUM(AGE)
FROM USER_INFO;

-- EMPLOYEE에서 부서코드가 'D5'인 사원들의 총 연봉
SELECT SUM(SALARY * 12)
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

SELECT 
    SUM(DECODE(DEPT_CODE, 'D5',SALARY * 12,0)) -- 그룹 함순 내에 조건문 가능
FROM EMPLOYEE;

SELECT
    SUM(CASE DEPT_CODE WHEN 'D5' THEN SALARY*12 END)
FROM EMPLOYEE;
...
SELECT
    MIN(SALARY), MIN(EMP_NAME), MIN(HIRE_DATE),
    MAX(SALARY), MAX(EMP_NAME), MAX(HIRE_DATE)
FROM EMPLOYEE;

/*
    COUNT -> 그룹 함수에서 가장 많이 사용
    - 컬럼 또는 행의 개수를 세서 반환
    - * : 조회 결과에 해당하는 모든 행 개수 반환
    - 컬럼 : 해당 컬럼값이 NULL 아닌 행 개수 반환
...
-- ROLLUP : 컬럼1 (DEPT_CODE) 기준으로만 합계
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
    FROM EMPLOYEE
    GROUP BY ROLLUP(DEPT_CODE, JOB_CODE);

-- CUBE : 컬럼1, 컬럼2 두가지 기준 모두 합계를 내줌
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
    FROM EMPLOYEE
...
/*
    ...
    - 여러 개의 쿼리문에서 조회하려고 하는 컬럼의 개수와 이름이 같아야
      ...
      INTERSECT (교집합) : 두 쿼리문을 수행한 결과 값에 중복된 결과값만 추출
...
-- 1. UNION
-- 부서코드가 D5 이거나 급여가 300만원 초과인 사원 조회 -> OR 연산자 사용해서 처리 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
...
-- 3. INTERSECT
-- 부서코드가 'D5'면서 급여가 300만원 초과인 사원들 조회 -> AND 연산자 사용가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
INTERSECT
