...
-- RIGHT JOIN (기준 삼는 테이블을 JOIN을 기준 LEFT , RIGHT)
SELECT EMP_NAME, DEPT_TITLE
    FROM EMPLOYEE 
...
/*
    ...
    - 테이블에 별칭을 주고, 다른 테이블로 인식시켜 조인
*/
-- 사원사번, 사원명, 사원부서코드, 사수사번, 사수명, 사수부서코드 조회
SELECT * FROM EMPLOYEE;
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, -- 사원 테이블
       M.EMP_ID,M.EMP_NAME, M.DEPT_CODE   -- 사수 테이블
       FROM EMPLOYEE E
       JOIN EMPLOYEE M ON (E.MANAGER_ID = M.EMP_ID);
       
/*
    카테시안곱(CATESIAN PROCUCT) / 교차 조인(CROSS JOIN)
...
SELECT * FROM EMPLOYEE; -- EMP_NAME, DEPT_CODE, JOB_CODE, SALARY 
SELECT * FROM JOB;  -- JOB_CODE, JAB_NAME
...
SELECT * FROM DEPARTMENT; -- DEPT_ID(E.DEPT_CODE), DEPT_TITLE, LOCATION_ID
SELECT * FROM JOB; -- JOB_CODE(E.JOB_CODE)
SELECT * FROM LOCATION; -- LOCAL_CODE(D.LOCATION_ID) NATIONAL_CODE
SELECT * FROM NATIONAL; -- NATIONAL_CODE(L.NATIONAL_CODE) , NATIONAL_NAME

SELECT LOCAL_NAME, EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, NATIONAL_NAME, SALARY
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
    JOIN JOB USING (JOB_CODE)
    JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
SELECT * FROM DEPARTMENT; -- DEPT_ID(E.DEPT_CODE), DEPT_TITLE
SELECT * FROM JOB; -- JOB_CODE(E.JOB_CODE)
...
SELECT EMP_NAME, EMP_NO, DEPT_TITLE, JOB_NAME
    ...
    JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
    JOIN JOB USING (JOB_CODE)
--    WHERE SUBSTR(EMP_NO,8,1)=2 AND EMP_NAME LIKE '전%' AND EMP_NO LIKE '7%'; 
...
SELECT DEPT_TITLE, TO_CHAR(SUM(SALARY),'999,999,999') 급여
    FROM DEPARTMENT
...
SELECT EMP_NAME, BONUS, JOB_CODE, JOB_NAME, SALARY
    FROM EMPLOYEE
    JOIN JOB USING (JOB_CODE)
    WHERE BONUS IS NULL AND JOB_CODE IN ('J4','J7');
...
SELECT TITLE, DIRECTOR, NAME
    FROM MOVIE_ACTOR -- 복합키로 묶은 테이블
    JOIN MOVIE USING(MOVIE_ID)
    JOIN ACTOR USING(ACTOR_ID);

-- 12. 평점이 3점 이상인 리뷰 영화 제목과 리뷰 쓴 사람, 평점 조회
SELECT TITLE, RATING, NAME, "COMMENT" --  컬럼 값이 문자열이라면 "컬럼"
    FROM MOVIE
...
SELECT TITLE, NAME, RATING
    FROM REVIEW -- 리뷰에 연결되어 있는 컬럼이 많음.
    JOIN USER_INFO USING(USER_ID)
    JOIN MOVIE USING(MOVIE_ID)
    WHERE RATING >=3;
...
-- 최동훈 감독영화
SELECT DISTINCT NAME -- 중복된 이름 제거 DISTINCT
    FROM MOVIE_ACTOR
    JOIN MOVIE USING (MOVIE_ID)
    JOIN ACTOR USING (ACTOR_ID)
    WHERE DIRECTOR = '최동훈'; 
...
SELECT ACTOR_ID FROM ACTOR WHERE NAME = '하정우'; -- ACTOR_ID = 3
...
WHERE ACTOR_ID = (SELECT ACTOR_ID FROM ACTOR WHERE NAME = '하정우');
...
SELECT TITLE, AVG(RATING), COUNT(MOVIE_ID)
    FROM MOVIE
    LEFT JOIN REVIEW USING (MOVIE_ID)
    WHERE RATING IS NOT NULL
    GROUP BY TITLE;
*/
SELECT TITLE, AVG(RATING),COUNT("COMMENT")
    FROM REVIEW
    JOIN MOVIE USING (MOVIE_ID)
    GROUP BY TITLE
