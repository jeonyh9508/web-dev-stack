/*
    ...
    * PL/SQL 구조
    ...
    - 실행부(EXECUTABLE SECTION) : BEGIN 으로 시작, SQL문 또는 제어문(조건문, 반복문)등의
                                 로직을 기술하는 부분
    - [예외처리부(EXCEPTION SECTION)] : EXCEPTION 시작, 예외 발생 시,
                                      해결하기 위한 구문을 미리 기술
*/
SET SERVEROUTPUT ON;
BEGIN
...
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE; -- EMP_ID의 타입으로
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('사번 ' || EID || '의 부서 : ' ||DTITLE);
END;
/
...
/*
    실행부
    제어문 (조건문, 반복문) 등의 로직을 기술

    1. IF 문
    IF 조건식 THEN 실행내용 END IF;
*/
-- 사번을 입력받은 후 해당 사원의 이름, 보너스 출력
-- 단, 보너스를 받지 않느 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다.' 출력
DECLARE
    EMP EMPLOYEE%ROWTYPE;
BEGIN
    SELECT *
...
/
/*
    테이블 - EMPLOYEE, 컬럼은 SALARY
    500만원 이상이면 '고급', 300만원 이상이면 '중급' 300만원 미만 이면 '초급'
    사원의 사번을 입력받아서 '해당 사원의 급여 등급은 고급 입니다.'
*/
...
BEGIN
    ...
    IF EMP.SALARY >= 5000000
        THEN GRADE := '고급'; 
    ELSIF EMP.SALARY >= 3000000
        THEN GRADE := '중급';
     ELSE GRADE := '초급';
    END IF;
    
    DBMS_OUTPUT.PUT_LINE('해당 사원 '|| EMP.EMP_NAME ||' 의 급여 등급은 '|| GRADE ||' 입니다.');
END;
/
/*
    CASE 비교대상자
    ...
    END CASE;
*/
-- 사번을 입력받은 후, 부서코드에 따라 D1 -> 개발팀, D2-> 디자인팀 D3 -> 기획팀, 그외 -> 서비스팀
-- 출력 : '사원명는 개발팀입니다'.
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('해당 사원 '|| EMP.EMP_NAME ||'은(는) ' || DNAME || '입니다.');
END;
/
/*
    반복문
...
/*
    ...
    * 오라클에서 미리 정의되어 있는 시스템 예외
    - NO_DATA_FOUND : SELECT한 결과가 한 행동 없는 경우
    - TOO_MANY_ROWS : SELECT한 결과가 한 행이 리턴되어야 하는 데 여러행인 경우
    - ZERO_DIVIDE : 숫자를 0으로 나눌 경우
...
-- EMPLOYEE에서 부서코드(DEPT_CODE)를 입력받아 사원을 조회하려 할 때 
