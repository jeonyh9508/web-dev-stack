/*
	...
	3. 동적 로딩 지원
		...
		- 메모리 효율
	4. 운영체제에 독립적 (JVM)
		- 자바 코드는 JVM 위에서 돌아가기 때문에 Window, macOSm Linux 모두 실행 가능
	...
	* 자바 프로그램 실행 단계
	1. 확장자가 .java 인 소스파일을 작성한다.
	2. 작성된 소스파일을 자바 컴파일러 (javac.exe) 로 컴파일하여
	   확장자가 .class 인 바이트코드(기계어) 로 번역
	3. 번역된 바이트코드(기계어)를 JVM의 자바 인터프리터(java.exe)를 이용해 실행
*/

/*
	패키지 (package)
	- 폴더를 만들어서 파일을 저장하고 관리하듯, 패키지를 만들어 클래스를 저장하고 관리
	- 패키지는 클래스를 유일하게 만들어주는 식별자 역항르 한다.
	- 패키지가 선언되지 않으면 default 패키지에 속하게 되는데, 실무에서는 거의 사용되지 않는다.
*/
